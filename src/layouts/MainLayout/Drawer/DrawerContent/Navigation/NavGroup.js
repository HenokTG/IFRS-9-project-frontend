...
const NavGroup = ({
  ...
  const { menuOrientation, isDrawerOpen, selectedNavID, setSelectedNavID } = useConfig();
  ...
  const checkSelectedOnload = (data) => {
    ...
    childrens.forEach((itemCheck) => {
      ...
      if (itemCheck.path === pathname) {
        setSelectedNavID(currentItem.id);
      }
    });
  };
  ...
  const itemIcon = currentItem?.icon ? (
    <Icon
      style={{
        fontSize: 20,
        stroke: '1.5',
        color: selectedNavID === currentItem.id ? theme.palette.primary.main : theme.palette.secondary.dark,
      }}
    />
  ) : null;
  ...
  return (
    <>
      {menuOrientation === MenuOrientation.VERTICAL || downLG ? (
        ...
        <List>
          <ListItemButton
            selected={selectedNavID === currentItem.id}
            sx={{
              p: 1,
              my: 0.5,
              mr: 1,
          ...
          >
            {itemIcon && (
              <ListItemIcon sx={{ minWidth: 28 }}>
                {currentItem.id === lastItemId ? (
                  <Iconify icon="ant-design:down-outlined" style={{ fontSize: 20, stroke: '1.5' }} />
                ) : (
                  itemIcon
                )}
              </ListItemIcon>
            ...
            />
            {openMini ? (
              <Iconify icon="ant-design:down-outlined" style={{ fontSize: 16, stroke: '1.5' }} />
            ) : (
              <Iconify icon="ant-design:right-outlined" style={{ fontSize: 16, stroke: '1.5' }} />
            )}
            {anchorEl && (
              <PopperStyled
                id={popperId}
